...
- `CHANGELOG.md` was deleted from the fork (upstream-only artifact; the fork
  records changes via commits). Upstream rewrites it every release, so each
  sync will re-add or conflict on it — resolve by taking upstream, then delete
  again if you still don't want it tracked.
